...
import {getCatalogData, setCatalogId} from '../../../store/actions/farmMeatActions';
    ...
    const interesting_questions = [
        {
         ...
         answer: 'Вы можете быть уверены в качестве нашего мяса. Вся наша мясная продукция проходит ветеринарный контроль. Документом является ветсвидетельство, которое выдается в электронном виде в системе Меркурий на каждую партию мяса, мы предоставим вам его по запросу.\n' +
             'Также есть декларации соответствия на мясо и готовую продукцию. Декларация подтверждает соответствие продукции требованиям безопасности, что говорит об отсутствии опасности для жизни и здоровья человека.\n' +
             'Наша цель  полезный и вкусный продукт для наших клиентов. Поэтому, чтобы вы были уверены в качестве продукта мы зарегистрировали собственный знак качества - Амбарцево.\n' +
             'Мы несем ответственность за каждый заказ, готовы вернуть деньги или обменять товар, если вас что-то не устроило.\n' +
             'Проверяйте наличие знака качества «Амбарцево» на вашем заказе.'
        },

        {
            id: 2,
            question: 'Доставка? ',
            answer: 'Доставка осуществляется бесплатно при заказе на сумму  от 3 000 руб., при заказе заказе до 3000р стоимость доставки составит 200 руб.\n' +
                ...
                'Таким образом вы получаете продукцию практически в день изготовления с запасом срока годности. \n' +
        ...
        },
        {
            ...
            answer: 'У нас исключительно натуральные, высококачественные фермерские продукты.\n' +
                'В наше время новых технологий откорма приходится очень избирательно относиться к выбору мясных продуктов для своей семьи. Мы распределяем части туши так, чтобы у каждого клиента были одинаковые равноценные части. Такое распределение дает возможность сохранять доступные цены и доставлять свежее мясо, не прибегая к глубокой заморозке и использованию консервантов.\n' +
                'В данный момент многие конкуренты копируют нашу идею.  \n' +
                'Чтобы быть уверенным,  рекомендуем проверять наличие знака качества “Амбарцево” на нашей продукции - мы несем ответственность за каждый заказ.'
        },
        {
            id: 4,
            question: 'Как заказать?',
            answer: '1. В меню внизу страницы выберите вкладку “Каталог”.\n' +
                '\n' +
                '2. Наполните корзину нужными вам товарами. Возможность перейти в корзину появится только когда вы добавите в нее что-то, это можно сделать из каталога или из карточки товара.\n' +
                '\n' +
                '3. Перейдите в корзину, чтобы оформить заказ. Укажите ваш адрес и выберите удобный вариант оплаты. Если у вас есть промокод, примените его в корзине до оформления заказа.\n' +
                '\n' +
                '4. Нажмите "Заказать". Статус заказа всегда можно посмотреть на вкладке “Профиль”, там же будет кнопка “Связаться с нами”, если необходимо что-то уточнить.\n' +
                '\n' +
                'PS. А ещё с 9.00 до 20.00 вы можете сделать заказ по телефону нажав на кнопку на главном экране.'
        },
        {
            id: 5,
            question: 'О нас ?',
            answer: 'Мы — команда “Фермерское мясо”, которая выросла в деревне и не понаслышке знает, что такое натуральные фермерские продукты! Всю нашу любовь к крестьянскому труду и здоровому питанию мы вложили в свою продукцию и этой любовью мы делимся с вами.\n' +
                'Мы начинали с небольших продаж мяса своим близким и знакомым, стараясь как для себя. Постепенно спрос увеличивался мы задумались как сохранить натуральность и сделать ее ближе городскому жителю, так появился формат мясного набора. \n' +
                'В данный момент мы сотрудничаем с фермерскими хозяйствами Нижегородской области, тщательно отбираем поставщиков, изготавливаем продукцию на собственном небольшом производстве. Принцип работы «как для себя» остался и остается до сих пор!'
        },
    ]
            ...
            <ScrollView style={styles.home_catalog_main_wrapper}>
                ...
                <View style={styles.home_catalog_search_input_delivery_phone_buttons_wrapper}>
                        <View style={styles.home_catalog_search_input_field_icon_wrapper}>
                            <View style={styles.home_catalog_search_icon}>
                                <SearchIcon/>
                            </View>
                            <TextInput
            ...
            {show_interesting_question_popup &&
                <View style={styles.interesting_question_popup}>
                    <View style={styles.interesting_question_popup_wrapper}>
                        ...
                        <Text style={styles.interesting_question_popup_text}>{active_interest.answer}</Text>
                    </View>
                </View>
            }
